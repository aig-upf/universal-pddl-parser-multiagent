(define (domain taxi)
 ...
 (:action drive
    ...
    :precondition  (and  (at ?t ?from)
        ...
        (free ?to)
        (forall (?t2 - taxi ?from2 - location) (not (drive ?t2 ?from2 ?to)))
        (forall (?t2 - taxi ?from2 - location) (not (drive ?t2 ?from2 ?from)))
        (forall (?p2 - passenger) (not (enter ?p2 ?t ?from)))
        (forall (?p2 - passenger) (not (exit ?p2 ?t ?from)))
      )
 ...
 (:action enter
    ...
    :precondition  (and  (at ?p ?l)
        (at ?t ?l)
        (empty ?t)
        (forall (?p2 - passenger) (not (enter ?p2 ?t ?l)))
        (forall (?p2 - passenger) (not (exit ?p2 ?t ?l)))
      )
