(define (domain wireless)
 ...
 (:predicates
  ...
  (not-message-at  ?m - message  ?n - node)      ;; message ?m is not at node ?n.
  (message-data  ?m - message  ?s - sensor)     ;; message ?m has data about the location of sensor node ?s.
  (not-message-data  ?m - message  ?s - sensor)     ;; message ?m has no data about the location of sensor node ?s.
  (sending   ?from - sensor  ?to - node ?m - message)  ;; sensor node ?from is sending message ?m to node ?to
  (not-sending   ?from - sensor  ?to - node ?m - message)  ;; sensor node ?from is not sending message ?m to node ?to
